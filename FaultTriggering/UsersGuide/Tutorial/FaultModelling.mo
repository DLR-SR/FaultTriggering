...
<p>In Figure 1. the driveline of a <a href=\"modelica://FaultTriggering.Examples.ActuatorExample.Parts.Drivelines.Driveline\">rotational</a> <a href=\"modelica://FaultTriggering.Examples.MotorExample.Driveline\">actuator</a> is shown. Three triggerable faults can be identified:</p>
<ol>
...
<p>A Boolean parameter fault can be modelled by using the <a href=\"modelica://FaultTriggering.FaultOutput.ConstantBooleanFault\">FaultTriggering.FaultOutput.ConstantBooleanFault</a> model and connecting it to the input of the fault case. In this case the boolean to real converter outputs 1 in case that the fault is false, and 0 otherwise. This engages or disconnects the driveline by disconnecting a <a href=\"modelica://Modelica.Mechanics.Rotational.Components.Clutch\">clutch</a>.</p>
<p>In the boolean parameter value block, it is possible to turn the fault on or off (default=off). <b>IMPORTANT:</b> Note that using an parameter to set the faults can lead to problems. See chapter &quot;<a href=\"modelica://FaultTriggering.UsersGuide.Tutorial.Important\">Important</a>&quot; for an explanation.</p>
...
<p>The variable faults are handled just like parameter faults. In Figure 1 the DriveFriction is modelled with a <a href=\"modelica://FaultTriggering.FaultOutput.FaultReal\">real variable fault</a> which is connected to a <a href=\"modelica://Modelica.Mechanics.Rotational.Components.Brake\">brake</a> element.</p>
<p>The fault model can be configured by the user (see Figure 3). Unlike the Parameter Fault where only the default value can be set, it is possible to set the record <b>externalRealFault</b> (see Figure 4). In this record three parameters must be set; The boolean <b>externalFaultOn</b> is used to switch the input from the global FaultTriggering on or off. This makes it possible to use the predefined default value without using the global model. The optional integer parameter <b>faultMode</b> can be used by advanced users to automatically reconfigure models. This reconfiguration is handled in the chapter <a href=\"modelica://FaultTriggering.UsersGuide.Tutorial.AutomaticModelReconfiguration\">Automatic Model Reconfiguration</a>. In the &quot;advanced&quot; tab the parameter <b>faultIndex</b>, which indicates which output channel of the global model <a href=\"modelica://FaultTriggering.FaultOutput.FaultTrigger\">FaultTrigger</a> is used. This should not be set by the user unless for debugging purposes or experienced users.</p>
<p><b>IMPORTANT:</b> Note that using an parameter to set the faults can lead to problems. See chapter &quot;<a href=\"modelica://FaultTriggering.UsersGuide.Tutorial.Important\">Important</a>&quot; for an explanation!</p>
...
<p>Using the models in the package <a href=\"modelica://FaultTriggering.FaultOutput.Internal\">FaultTriggering.FaultOutput.Internal</a>, it is possible to create broken models by using the &quot;<i>extends</i>&quot; functionality. An example of this possibility can be found in the sensor model <a href=\"modelica://FaultTriggering.Examples.MotorExample.FaultSpeedSensor\">FaultTriggering.Examples.MotorExample.FaultSpeedSensor</a> </p>
